...
A symmetric `dim × dim` matrix of class `covMat`. Either correlation or
covariance depending on `corr`.
...
- "factor":
  ...
  `loadings` (matrix dim × k), `psi` (vector of uniquenesses length
  dim).

- "graph":

  `adjacency` (symmetric matrix dim × dim).

- "spatial":

  `distances` (dim × dim matrix), `kernel` (function or string:
  `"exponential"`, `"gaussian"`, `"matern"`, `"rational_quadratic"`,
  `"powered_exponential"`, `"cauchy"`), kernel-specific parameters
  (`phi`, `nu`, `alpha`, `beta`, `power`).
...
**Time series types:** Non-stationary models use MA(∞) expansion with
differencing. Roots of AR/MA polynomials are stabilized for
invertibility.
...
Default base: a dim × rank_target Vandermonde-like matrix of sin(0:π/2)
values, scaled ×10
...
``` r
# Diagonal covariance
construct_covMatrix(5, type = "diag", params = list(variances = 1:5))
...
# AR(1) correlation
construct_covMatrix(6, corr = TRUE, type = "AR", params = list(rho = 0.8))
#> The constructed matrix is PD.
#>         [,1]   [,2]  [,3]  [,4]   [,5]    [,6]
...
# Block matrix
construct_covMatrix(5, type = "block", params = list(groups = c(2,3), rho = c(0.5, 0.3)))
#> The constructed matrix is PD.
#>      [,1] [,2] [,3] [,4] [,5]
...
# Factor model
L <- matrix(rnorm(10), 5, 2); ψ <- rep(0.2, 5)
construct_covMatrix(5, type = "factor", params = list(loadings = L, psi = ψ))
#> The constructed matrix is PD.
#>            [,1]        [,2]       [,3]       [,4]        [,5]
...
D <- as.matrix(dist(matrix(runif(10), ncol = 1)))
construct_covMatrix(5, type = "spatial", params = list(distances = D, kernel = "matern", phi = 1, nu = 0.5))
#> Error in construct_covMatrix(5, type = "spatial", params = list(distances = D,     kernel = "matern", phi = 1, nu = 0.5)): `distances` must be a numeric square matrix of size dim × dim

# Spectrum-constrained correlation matrix
construct_covMatrix(4, type = "specConstr", params = list(eigenvalues = c(2,1,0.5,0.5)))
#> The constructed matrix is PD.
#>           [,1]      [,2]      [,3]      [,4]
